## Harbor <https://github.com/wch/harbor> looks like the most promising package for controlling
## Docker using R. This is a quick attempt at baisc functionality - once that package is published
## on CRAN then most of this can go.
...
#' }
docker_cmd <- function(host, cmd = NULL, args = NULL, docker_opts = NULL, capture_text = FALSE, ...) {

  con <- docker_cmd_con(host, cmd, args, docker_opts, ...)
  on.exit(close(con))
...
#'
docker_cmd_con <- function(host, cmd = NULL, args = NULL, docker_opts = NULL, ...) {
  UseMethod("docker_cmd_con")
}
...
docker_run <- function(host = localhost, image = NULL, cmd = NULL,
  ...
  if (is.null(name)) {
    name <- random_name(prefix = "resurrectr")
  }
